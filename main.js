let container = document.querySelector('.myGHInfo');
let bio = document.querySelector('.bio');
let pic = document.querySelector('.pic');

let request = new XMLHttpRequest();
request.addEventListener("load", myGHInfo);
request.open('GET', 'https://api.github.com/users/georgiatardy');
request.send();
...
function myGHInfo() {
  ...
  info += `<li><b>Name:</b>  ${data.name}</li>`;
  info += `<li><b>Github URL: <a href="${data.html_url}"" target="_blank"></b> georgiatardy</a></li>`;
  info += `<li><b>Location:</b>  ${data.location}</li>`;
  info += `<li><b>Company:</b>  ${data.company}</li>`;
  info += `<li><b>Website: <a href="${data.blog}" target="_blank"></b>www.georgiatardyart.com</a></li>`;
  paragraph += `<p>${data.bio}</p>`;
  image += `<img src= "${data.avatar_url}">`;
  container.innerHTML = info;
  bio.innerHTML = paragraph;
  pic.innerHTML = image;
...
//======Code before using Template Literals=======//
